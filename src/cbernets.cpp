...
work (void *ptr)
{
  ...
  printf ("Task[%d] done, tasks done until now = %d, total_tasks = %d\n",
	  p.t->i, p.t->current_job->done, p.t->current_job->size);
  pthread_mutex_unlock (&p.t->current_job->mutex);
  return NULL;
}

void
sched_jobs (std::vector < Job > j_vec, std::vector < Pod > p_vec)
  //schedule jobs from the smallest tasks
  //always assign the job that makes more ram
  //available comparing both Pods
{
  int highest_sum = 0;
...
#if DEBUG == 1
  std::cout << "smallest ram = " << j_vec[0].get_smallest_task_ram ()->
    ram << std::endl;
  std::cout << "biggest ram = " << j_vec[0].get_biggest_task_ram ()->
    ram << std::endl;
  std::cout << "smallest cpu = " << j_vec[0].get_smallest_task_cpu ()->
    cpu << std::endl;
  std::cout << "biggest cpu = " << j_vec[0].get_biggest_task_cpu ()->
    cpu << std::endl;
#endif
  while (1)
    {
      for (int i = 0; i < p_vec.size (); i++)
      {
          printf ("p_vec[%d] = .ram=%d, .cpu=%d\n",
          i, p_vec[i].ram, p_vec[i].cpu);
      }
      sleep (1);
    };
